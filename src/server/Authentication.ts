...
export async function verifyClientToken(
  token: string,
  allowPrivateGuest = false,
): Promise<TokenVerificationResult> {
  if (PersistentIdSchema.safeParse(token).success) {
    if (authPolicy().allowPrivateGuests && allowPrivateGuest) {
      // The browser UUID is a secret bearer credential, never an account ID.
      // Hash into a separate identity so submitting a known account UUID cannot
      // impersonate that account or inherit its host permissions.
      const bytes = createHash("sha256")
        .update(`openfront-private-guest:${token.toLowerCase()}`)
        .digest();
      bytes[6] = (bytes[6] & 0x0f) | 0x50;
      bytes[8] = (bytes[8] & 0x3f) | 0x80;
      const hex = bytes.subarray(0, 16).toString("hex");
      const persistentId = `${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20)}`;
  ...
  try {
    ...
    if (
      authPolicy().verifySessionOnline &&
